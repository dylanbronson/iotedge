    ...
    public class DeviceIdentityProvider : IDeviceIdentityProvider
    {
        ...
        public async Task<IDeviceIdentity> GetAsync(string clientId, string username, string password, EndPoint clientAddress)
        {
            try
            {
                ...
                return new ProtocolGatewayIdentity(deviceCredentials, deviceCapabilityModelId);
            }
            catch (Exception ex)
            {
        ...
        internal static (string deviceId, string moduleId, string deviceClientType, Option<string> deviceCapabilityModelId) ParseUserName(string username)
        {
            ...
            Option<string> deviceCapabilityModelId = Option.None<string>();
            if (queryParameters.TryGetValue(DeviceCapabilityModelIdKey, out string dcmid))
            {
                if (!string.IsNullOrWhiteSpace(dcmid))
                {
